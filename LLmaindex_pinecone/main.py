...
async def startup_event():
    ...
    try:
        ...
        log.info(f"📄 Loaded {len(docs)} docs")
        ...
        log.info(f"🧩 Chunked to {len(nodes)} nodes")
        ...
        log.info(f"✨ Stored {len(nodes)} vectors in Pinecone")
        log.info(f"🔐 Pinecone index ready in {duration} sec")
        log.info("🚀 FAST RAG Engine Ready (LlamaIndex + Pinecone)")
